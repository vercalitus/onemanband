...
import { AR_MESSAGES } from "@/lib/i18n/translations-ar"

/** English is authoritative — Hebrew/Arabic overlay only supplied keys. */
const EN_FLAT = EN_MESSAGES as Record<string, string>
const HE_MERGED = { ...EN_FLAT, ...HE_MESSAGES }
const AR_MERGED = { ...EN_FLAT, ...AR_MESSAGES }
...
/** Build a translator; missing locale keys fall back to English. */
export function createTranslator(locale: Locale): TranslateFn {
  const table =
    locale === "he" ? HE_MERGED : locale === "ar" ? AR_MERGED : EN_FLAT
